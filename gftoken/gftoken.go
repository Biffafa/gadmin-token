...
type GfToken struct {
	// server name
	ServerName string
	// 缓存 key（每创建一个实例 CacheKey 必须不同）
	CacheKey string
	// 超时时间 默认 10 天（秒）
	Timeout int64
	// 缓存刷新时间 默认 5 天（秒）
	// 处理携带 token 的请求时当前时间大于超时时间并小于缓存刷新时间时 token 将自动刷新即重置 token 存活时间
	// MaxRefresh 值为 0 时，token 将不会自动刷新
	MaxRefresh int64
	// 是否运行多点登录
	MultiLogin bool
	// Token 加密 key 32 位
...
func (m *GfToken) GenerateToken(ctx context.Context, key string, data interface{}) (keys string, err error) {
	...
	if m.MultiLogin {
		...
		if tData != nil {
			...
			m.doRefresh(ctx, key, tData) // 刷新 token
			return
		}
	}
	tokens, err = m.userJwt.CreateToken(CustomClaims{
		...
		jwt.RegisteredClaims{
			...
			ExpiresAt: jwt.NewNumericDate(m.diedLine()),
		},
	})
	if err != nil {
		return
...
// ParseToken 解析 token (只验证格式并不验证过期)
